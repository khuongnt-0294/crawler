...
class DataWorker2
  ...
  def perform
    ...
    @list_page = @agent.get("http://doda.jp/DodaFront/View/JobSearchList.action?so=50&tp=1&pic=1&ds=0&page=100")
    @test = 0
    ...
    begin
      ...
      (0..@news.count-1).each do |i|              
        ...
        if !(Job.pluck(:url).include?(@form_uri))
          @job = Job.new
          @form_inexperience = ""
          ...
          if !(Company.pluck(:name).include?(@form11))
            @company = Company.new
            ...
            # @company = Company.find_by_sql "SELECT * FROM thieve_development.companies WHERE name='#{@form11}'"
          end          
          @job.company = @company
          @job.title = @form1
    ...
    end while ((@list_page.link_with(text: "次へ").present? || @test == 1) && (@list_page.uri.to_s.strip != "http://doda.jp/DodaFront/View/JobSearchList.action?so=50&tp=1&pic=1&ds=0&page=150"))
  end
