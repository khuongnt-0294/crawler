...
class DataWorker4
  ...
  def perform
    ...
    @list_page = @agent.get("http://doda.jp/DodaFront/View/JobSearchList.action?so=50&tp=1&pic=1&ds=0&page=200")
    @test = 0
    ...
    end while ((@list_page.link_with(text: "次へ").present? || @test == 1) && (@list_page.uri.to_s.strip != "http://doda.jp/DodaFront/View/JobSearchList.action?so=50&tp=1&pic=1&ds=0&page=250"))
  end
